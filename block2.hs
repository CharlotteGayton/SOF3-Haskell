...
Passes in a list of numbers and returns the even ones plus one 
-}
oneone, oneone' :: [Int] -> [Int]
oneone ns = [ n+1 | n <- ns,  n `mod` 2 == 0 ]
oneone' ns = map (+1) (filter (f) ns)
    where f n = n `mod` 2 == 0

oneone'' ns = map (+1) (filter ((== 0) . flip mod 2) ns)

oneone''' = map (+ 1) . (filter ((== 0) . flip mod 2))
